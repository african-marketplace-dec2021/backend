...
const {isEmptyArray, isUndefined, verifyInterger, processBodyToObject, verifyStringAndLength, verifyDecimal, verifyString, verifyInteger} = require("../helper");
...
async function verifyNewObject (req, res, next){
    try{
        ...
        }else if(verifyInteger(price) === false){
...
async function verifyModifiedObject (req, res, next){
    try{
        ...
        if(!isUndefined(name) && verifyStringAndLength(name, 3, 30) === false){
            ...
            res.status(400).json({message:"description must be string beteen3 and 1000 characters long"});
        }else if(!isUndefined(price) && verifyInteger(price) === false){
